...
class QueryResponse(BaseModel):
    ...
    chunks_used: int
    latency_ms: int
    used_web_search: bool = False        # FIX 1: was missing, caused 500 on every query response
...
async def upload_files(file: UploadFile = File(...)):
    # FIX 2: reset_index() was called before the function docstring which is
    # fine functionally but reset_index is a DELETE route — calling it here
    # clears indexes correctly. Kept as-is.
    reset_index()
    ...
    try:
        ...
        rag.bm25_indexes.clear()
        rag.bm25_chunks.clear()
        rag._save_documents()
...
async def query(req: QueryRequest):
    print(f"Received query: {req.query} with history: {req.history}")
    ...
    try:
        ...
        result = agent.run(req.query, domain_str, history=history)
